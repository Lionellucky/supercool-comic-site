...
// === PATCHED BBPLAYER.JS ===
// This assumes bbplayer.js initializes audio and switches tracks.
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const audio = bbplayer.querySelector('audio');
  const albumCover = bbplayer.querySelector('.bb-albumCover');

  function updateAlbumCover() {
    const sources = audio.getElementsByTagName('source');
    for (let src of sources) {
      if (audio.currentSrc.includes(src.getAttribute('src'))) {
        const image = src.getAttribute('data-image');
        if (image) {
          albumCover.style.backgroundImage = `url('${image}')`;
        }
        break;
      }
    }
  ...
  audio.addEventListener('loadeddata', updateAlbumCover);
  updateAlbumCover(); // Initial call

  // You might already have custom next/prev track logic
  // Ensure updateAlbumCover() is called after switching tracks
});